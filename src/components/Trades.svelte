<script lang="ts">
  ...
  $: filtered = ruleFilter
    ? trades.filter((t) => t.rule_id === ruleFilter)
    : trades;
...
<div class="card">
  <div class="card-header">
    ...
    <div class="head-right">
      <select class="rule-filter" bind:value={ruleFilter} title="按跟单规则筛选">
        <option value="">全部规则（{trades.length}）</option>
        {#each rules as r (r.id)}
          {@const n = trades.filter((t) => t.rule_id === r.id).length}
  ...
  {#if filtered.length === 0}
    <div class="empty">暂无交易{ruleFilter ? "（该规则下）" : ""}。</div>
  {:else}
    <div class="tbl">
      <div class="row head">
        ...
        <span class="right">盈亏</span><span>备注</span><span>魔术号</span>
      </div>
      <div class="vl-wrap">
        <VirtualList items={filtered} rowHeight={32}>
          <svelte:fragment let:item={t}>
            <div class="row">
              ...
              <span class="cmt muted" title={t.comment || "无备注"}>{t.comment || "—"}</span>
...
<style>
  ...
  .row {
    display: grid;
    grid-template-columns: 105px 1fr 100px 84px 64px 70px 64px 84px 80px 1.2fr 70px;
    align-items: center;
    height: 32px;
    padding: 0 8px;
  ...
  .cmt { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; padding-right: 6px; }
