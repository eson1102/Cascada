...
void DoOpenMarket(const string line)
{
   ...
   int    magic = (int)StringToInteger(JsonField(line, "magic"));
   ...
   int r = OrderSend(sym, t, vol, price, pts, sl, tp, cmt, magic, 0, clrNONE);
...
void DoOpenPending(const string line, bool is_limit)
{
   ...
   int r = OrderSend(sym, t, vol, tgt, 5, sl, tp, cmt, magic, exp, clrNONE);
...
void SaveOrigins()
{
   ...
   for(int i = 0; i < ArraySize(g_origins); i++)
      FileWriteString(h, IntegerToString(g_origins[i].ticket)
                        + "|" + g_origins[i].origin + "\n");
   FileClose(h);
}
...
void LoadOrigins()
{
   ...
   while(!FileIsEnding(h))
   {
      ...
      if(pipe > 0)
      {
         int t = (int)StringToInteger(StringSubstr(line, 0, pipe));
...
string JsonField(const string s, const string key)
{
   ...
   if(StringGetCharacter(s, i) == '"')
   {
      ...
      while(end < len)
      {
         ...
         if(c == '"') break;
         if(c == '\\' && end + 1 < len) { end += 2; continue; } // skip \"
         end++;
      }
